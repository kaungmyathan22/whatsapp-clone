...
export default function ChatMessages ({ messages, user, roomId, audioId, setAudioId }) {
  ...
  return messages.map(message => {
    ...
    return <div key={message.id} className={`chat__message ${isSender ? 'chat__message--sender' : ''}`}>
      <span className='chat__name'>
        {message.name}
      </span>
      {message.imageUrl === "uploading" ? <div className="image-container">
        <div className='image-container--loader'>
          <CircularProgress style={{ width: 40, height: 40 }} />
        </div>
      </div> : message.imageUrl ? <div className='image-container'>
        <img src={message.imageUrl} alt={message.name} />
      </div> : null}
      {message.audioName ? (<AudioPlayer
        sender={isSender}
        roomId={roomId}
        id={message.id}
